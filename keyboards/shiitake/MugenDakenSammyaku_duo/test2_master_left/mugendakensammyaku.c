...
report_mouse_t mouse_mode_task(int input_x, int input_y, int btn_num, bool shift_flag){
    ...
    shift_trigger = false;
    if(shift_flag){
        if(deadzone_checker(input_x, input_y, (2 * MOUSE_DEADZONE / 3))){
            shift_trigger = true;
        }
    }
...
report_mouse_t scroll_mode_task(int input_h, int input_v, bool ctrl_flag){
    ...
    ctrl_trigger = false;
    if(ctrl_flag){
        if(deadzone_checker(input_h, input_v, (2 * SCROLL_DEADZONE / 3))){
            ctrl_trigger =true;
        }
    }
...
void dpad_mode_task(int input_x, int input_y, int lr_flag){
    ...
    float each_axis_deadzone = DPAD_DEADZONE / sqrt(2);
...
report_mouse_t fusion_mode_task_all(int input[2][2], int lr_flag){
    ...
    int mean_input[3] = {
        ...
        (input[0][1] + input[1][1]) /2,
        (abs(input[0][0]) + abs(input[1][0])) /2
    };
